<template>
  <div>
    <base-skeleton class="mt-3" animated :loading="loading">
      ...
      <template #default>
        <div>
          <h2 class="text-2xl my-4 text-gray-600">ویرایش ویژگی</h2>
        </div>
        <div class="bg-white shadow-xl rounded-xl p-4">
          <base-form
            @submit.prevent="handleUpdateFeature"
            :model="form"
            ref="formRef"
  ...
  formRef.value?.validate(async (valid: any): Promise<void> => {
    if (valid) {
      ...
      try {
        ...
        if (form.value.cover.file) {
          formData.append("cover", form.value.cover?.file);
        }
        if (form.value.icon.file) {
          formData.append("icon", form.value.icon?.file);
        }
        const data = await useApiService.put(
          `management/features/${service_id.value}`,
          formData
        );

        if (data.success) {
          BaseMessage({
            message: "ویرایش ویژگی با موفقیت انجام شد!",
            type: "success",
            duration: 4000,
            center: true,
            offset: 20,
