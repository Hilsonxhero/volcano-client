      ...
      <template #default>
        <section class="mb-6 bg-white shadow-lg p-4 rounded-2xl">
            ...
            <div class="mb-3 flex justify-between flex-wrap items-center">
              ...
              <div class="w-full lg:w-auto mt-4 lg:mt-0">
                <base-button
                  ...
                  :to="{ name: 'management-pages-create' }"
                >
                  <div class="flex items-center">
                    <span class="ml-2"> ایجاد صفحه</span>
          ...
          <BaseDataTable
            ...
            search-placeholder="جستجوی صفحه"
          ...
          >
            ...
            <template v-slot:cell-actions="{ row: page, index }">
              <div class="flex items-center">
                <base-button
...
const fetchPages = async () => {
  ...
  try {
    const { data } = await useApiService.get(`management/pages`, {
      params: params,
    });
    loading.value = false;
...
const debouncedOnInputChange = debounce(fetchPages, 200);
...
const handleDeletePage = (page: any, index: any) => {
  BaseMessageBox.confirm(`آیا از حذف  صفحه  اطمینان دارید ؟!`, "پیام تایید", {
    ...
    .then(async () => {
      const data = await useApiService.remove(`management/pages/${page?.id}`);
      if (data.success) {
