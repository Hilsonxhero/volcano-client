<template>
  <div>
    <base-dialog @close="handleClose" v-model="visible_time_dialog" title="ثبت زمان">
      <div>
        <base-form :model="issue_time" ref="formRef" class="h-full space-y-4">
          <base-form-item :model="issue_time" prop="spent_on" label="در تاریخ" class="col-span-12 lg:col-span-4">
            <client-only placeholder="loading...">
              <date-picker v-model="issue_time.spent_on" type="datetime" format="jYYYY/jMM/jDD"></date-picker>
            </client-only>

            <BaseValidationError :errors="validation_errros" field="spent_on" />
          </base-form-item>
          <base-form-item :model="issue_time" prop="hours" label="ساعت" class="col-span-12 lg:col-span-4" :rules="[
            {
              required: true,
          ...
          <base-form-item :model="issue_time" prop="description" label="توضیحات" class="col-span-12 mt-4">
            <base-input class="h-[80px] resize-none" input-class="resize-none" type="textarea"
              v-model="issue_time.description" placeholder=" توضیحات "></base-input>
            <BaseValidationError :errors="validation_errros" field="description" />
          </base-form-item>
          <base-form-item :model="issue_time" prop="project_time_category_id" :rules="[
            {
              required: true,
              message: '  دسته بندی زمان  الزامی می باشد',
            },
          ]" label="دسته بندی زمان" class="col-span-12">
            <base-select v-model="issue_time.project_time_category_id" filterable placeholder="دسته بندی زمان"
              value-key="id" label="title" :options="project_time_categories">
            </base-select>
            <BaseValidationError :errors="validation_errros" field="project_time_category_id" />
          </base-form-item>
        </base-form>
      </div>
      <template #footer>
        <div class="flex flex-col justify-between lg:items-center mt-8">
          <div class="w-full flex items-center">
            <base-button @click="handleSubmitTime" class="" :loading="loader" type="primary" block>
              ایجاد
            </base-button>
            <base-button @click="visible_time_dialog = false" class="mr-2">
              لغو
            </base-button>
          </div>
        </div>
